...
const LoanApprovalDetail = () => {
  ...
  const validateSubmission = () => {
    ...
    if (!hasAnyDecision) {
      setSnackbar({
        open: true,
        message:
          "Please make a decision (approve/reject) for at least one item",
        severity: "error",
      });
      return false;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const itemsToProcess = loanDetails.items;
      ...
      <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
        ...
        >
          <CardContent>
            ...
            <Typography className="mb-2">
              <strong>NIM/NIK/NIDN:</strong> {loanDetails.nik}
            </Typography>
            <Typography className="mb-2">
          ...
          >
            <FormControl
              variant="outlined"
              sx={{ marginRight: 2, minWidth: 120, width: "100%" }}
            >

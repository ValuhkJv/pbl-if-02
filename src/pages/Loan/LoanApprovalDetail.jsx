...
const LoanApprovalDetail = () => {
  ...
  const validateSubmission = () => {
    ...
    if (invalidRejections.length > 0) {
      sweetAlert.error(
        "Error",
        "Please provide rejection reasons for all rejected items"
      );
      return false;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const token = sessionStorage.getItem("token");
      const pendingItems = loanDetails.items.filter(
        (item) => item.status === "pending"
      );
      const itemsToProcess = pendingItems.filter((item) => {
        return (
          itemApprovals[item.borrowing_id] !== undefined ||
      ...
      for (const item of itemsToProcess) {
        ...
        if (isApproved !== undefined || rejectionReason) {
          const response = await fetch(
            `http://localhost:5000/peminjaman/${borrowing_id}/approval`,
            {
              method: "PUT",
              headers: {
          ...
          if (!response.ok) {
            throw new Error(
              responseData.message || "Failed to process approval"
            );
          }
        }
      }
      ...
      sweetAlert.success(`Berhasil memperbarui peminjaman`);
          ...
          >
            <CardContent>
              ...
              <Typography className="mb-2">
                ...
                {new Date(loanDetails.borrow_date).toLocaleDateString()}
              </Typography>
              <Typography className="mb-2">
                <strong>Tanggal Pengembalian:</strong>{" "}
                {loanDetails.return_date
                  ? new Date(loanDetails.return_date).toLocaleDateString()
                  : "-"}
              </Typography>
            </CardContent>
              ...
              {displayedRows.map((item, index) => (
                <StyledTableRow key={item.item_code}>
                  ...
                  <StyledTableCell>
                    {" "}
                    {item.status.startsWith("return: terlambat") ? (
                      <Typography color="error">{item.status}</Typography>
                    ) : (
                      item.status
                    )}
                  </StyledTableCell>
                  ...
                  <StyledTableCell>
                    {isItemPending(item.status) &&
                    !itemApprovals[item.borrowing_id] ? (
                      <TextField
                        fullWidth
                        size="small"

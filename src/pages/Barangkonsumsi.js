...
export default function BarangKonsumsiTable() {
  ...
  const handleDeleteBarang = (id) => {
    fetch(`http://localhost:5000/barang-konsumsi/${id}`, {
      method: "DELETE",
    })
      .then((res) => {
        if (res.ok) {
  ...
  const handleSave = () => {
    const url = isEdit
      ? `http://localhost:5000/barang-konsumsi/${formData.id}`
      : "http://localhost:5000/barang-konsumsi";
    ...
    <Grid>
      ...
      >
        ...
        <TablePagination
          ...
          component="div"
          count={filteredRows.length} // Ubah dari rows.length ke filteredRows.length

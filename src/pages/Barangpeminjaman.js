...
export default function BarangpeminjamanTable() {
  ...
  const fetchData = () => {
    fetch("http://localhost:5000/barang-peminjaman")
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleDeleteBarang = (id) => {
    fetch(`http://localhost:5000/barang-peminjaman/${id}`, {
      method: "DELETE",
    })
      .then((res) => {
        if (res.ok) {
  ...
  const handleSave = () => {
    const url = isEdit
      ? `http://localhost:5000/barang-peminjaman/${formData.id}`
      : "http://localhost:5000/barang-peminjaman";
              ...
              {filteredRows
                ...
                .map((row, index) => (
                  <TableRow hover role="checkbox" tabIndex={-1} key={row.id}>
                    ...
                    <TableCell align="center">{row.no_inventaris}</TableCell>
      ...
      >
        ...
        <DialogContent>
          <TextField
            ...
            onChange={(e) =>
              setFormData({ ...formData, no_inventaris: e.target.value })
            }
            margin="dense"
            disabled={isEdit}
            required
